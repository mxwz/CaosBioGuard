...
本边缘端核心代码（包含 UI 与本地推理逻辑）采用 **[Apache License 2.0](LICENSE)** 协议开源。

由于使用了 `PySide6` (LGPLv3)，在进行商业分发时，请注意：
*   如果您将本程序打包成独立软件（如 `.exe`）或烧录至硬件设备中对外销售，请确保遵循 LGPLv3 关于动态链接和用户可替换库的相关开源义务。