...
class ArcFaceUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        title_label = QLabel("ArcFace 人脸识别系统 (Pi)")
    ...
    def display_image(self, frame_rgb):
        ...
        # 确保输入是 RGB 格式
        # 因为在调用此方法之前，很多地方已经做了 cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
        # 所以这里不需要再转换，直接使用即可。如果是BGR进来，则需要转换。
        # 这里的假设是：所有调用 display_image 的地方传入的都是 RGB 格式的帧。
        # 我们可以通过参数名 frame_rgb 来明确这一点。
