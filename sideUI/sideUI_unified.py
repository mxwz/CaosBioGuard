#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2025/11/5 下午2:16
# @Author  : mxwz
# @Software: PyCharm
# @File    : sideUI_unified.py
# @Desc    : 整合版，支持同步和异步模式切换
# @Improve : 添加人脸对比功能，配置切换启动模式
import logging
import sys
import pyttsx3
import os
...
class AdminLoginDialog(QDialog):
    ...
    def update_face_frame(self):
        if self.cap and self.cap.isOpened() and self.face_worker:
            ret, frame = self.cap.read()
            if ret:
                # 进行人脸识别
                try:
                    faces = self.face_worker.get(frame)
                    if len(faces) > 0:
                        # 获取最佳匹配
...
# SyncDatabaseManager moved to managers.py


# AsyncDatabaseManager moved to managers.py
...
class ManageFacesDialog(QDialog):
    ...
    def delete_selected(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            has_local = False
            for f in user_faces:
                if f.get('device_id', 'admin') == local_device_id:
                    has_local = True
                    break
...
class FaceComparisonDialog(QDialog):
    ...
    def update_frame(self):
        ...
        if self.cap and self.cap.isOpened():
            ...
            if ret:
                ...
                # 显示帧
                self.display_frame_on_label(frame, self.camera_label)
    ...
    def capture_and_compare(self):
        ...
        if self.face_worker:
            ...
            face_roi = self.current_frame[bbox[1]:bbox[3], bbox[0]:bbox[2]]

            # 显示拍摄的人脸
            self.display_frame_on_label(face_roi, self.captured_label)

            # 进行人脸比对
            # Note: We need to use local device ID to find the best match image since it's saved locally
            local_device_id = getattr(self.database_manager, 'device_id', 'admin')
            best_match, best_similarity, _, _ = self.database_manager.find_best_match(embedding, device_id=local_device_id)
            ...
            if best_match and best_similarity > 0.6:
                ...
                # 尝试加载匹配的人脸图像
                matched_image = self.database_manager.load_face_image(best_match, device_id=local_device_id)
                if matched_image is not None:
                    self.display_frame_on_label(matched_image, self.matched_label)
                else:
    ...
    def check_missing_images(self):
        ...
        for name in names:
            if not self.database_manager.check_face_image_exists(name, device_id=local_device_id):
...
class RetakeImagesDialog(QDialog):
    ...
    def capture_and_save(self):
        ...
        if self.face_worker:
            ...
            local_device_id = getattr(self.database_manager, 'device_id', 'admin')
            self.database_manager.save_face_image(name, face_roi, device_id=local_device_id)
...
# ConfigManager moved to managers.py
...
class ArcFaceUI(QMainWindow):
    def __init__(self):
        ...
        self.config_manager = ConfigManager(os.path.join(project_root, "config.ini"))
        
        # 根据配置选择启动模式
        start_mode = self.config_manager.get_start_mode()
        self.is_async = (start_mode == 'Async')
        
        db_path = os.path.join(base_dir, "dd", "face_database.pkl")
        sqlite_path = os.path.join(base_dir, "dd", "records.db")
        ...
        try:
            config_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                       "sideUI", "cfgs", "mobilenetv2.yaml")
            model_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                      "sideUI", "models", "_14_best.pth.tar")
            print(f"加载活体检测模型: {config_path}, {model_path}")
    ...
    def init_ui(self):
        ...
        title_label = QLabel("ArcFace 人脸识别系统")
        ...
        register_layout.addRow(self.register_image_btn)
        ...
        register_layout.addRow(self.capture_btn)
        register_layout.addRow(self.cancel_register_btn)

        # 识别区域
        recognition_group = QGroupBox("人脸识别")
        recognition_layout = QVBoxLayout(recognition_group)

        self.start_camera_btn = QPushButton("开始摄像头识别")
        self.start_camera_btn.clicked.connect(self.start_camera_recognition)
        recognition_layout.addWidget(self.start_camera_btn)

        self.stop_camera_btn = QPushButton("停止摄像头识别")
        self.stop_camera_btn.clicked.connect(self.stop_camera_recognition)
        self.stop_camera_btn.setEnabled(False)
        recognition_layout.addWidget(self.stop_camera_btn)

        # 人脸比对按钮
        self.face_comparison_btn = QPushButton("人脸比对")
        self.face_comparison_btn.clicked.connect(self.open_face_comparison_dialog)
        recognition_layout.addWidget(self.face_comparison_btn)

        # 管理人脸区域
        manage_group = QGroupBox("人脸管理")
        ...
        # 添加控件到控制面板
        control_layout.addWidget(register_group)
    ...
    def _perform_image_registration(self):
        ...
        # 检查姓名是否已存在
        # Actually, in multi-device mode, we allow same name but different device.
        # But for local DB, check if it exists on THIS device.
        user_faces = self.database_manager.database.get(name, [])
        local_exists = any(f.get('device_id', 'admin') == getattr(self.database_manager, 'device_id', 'admin') for f in user_faces)
    ...
    def update_frame(self):
        if self.cap and self.cap.isOpened():
            ...
            if ret:
                ...
                # 处理帧
                faces = self.face_worker.process_frame(frame)
                                ...
                                try:
                                    ...
                                    if face_roi.size > 0:  # 确保人脸区域不为空
                                        ...
                                        if probability[1] >= threshold:
                                            ...
                                            # 进行人脸识别
                                            self.perform_recognition(embedding, bbox, frame)
                                        else:
    ...
    def update_recognition_result(self, name, similarity, groups='all', list_type='white'):
        ...
        if not is_allowed:
             # 虽然识别出人脸，但不属于当前业务组，视为陌生人或无权限
             # 可以选择不显示名字，或者显示“无权限”
             self.video_label.setStyleSheet("background-color: black;") # 橙色警告
    ...
    def capture_image(self):
        """拍照按钮的处理函数"""
        if hasattr(self, 'registering_name') and self.cap and self.cap.isOpened():
            ret, frame = self.cap.read()
            if ret:
                faces = self.face_worker.process_frame(frame)
                if len(faces) > 0:
    ...
    def capture_for_registration(self, embedding):
        ...
        if hasattr(self, 'registering_name'):
            ...
            # 获取当前帧（云边分离模式下 add_face 可能离线拒绝，捕获后提示）
            try:
                ret, frame = self.cap.read()
                if ret:
                    # 处理帧以获取人脸位置
